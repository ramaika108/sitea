  ...
  return (
    <div className={`pagination pagination--${color} ${extraClass}`}>
      {pagination?.current === 1 ? (
        ...
        <div onClick={() => changePage(-1)} className="pagination-item">
          &lt;
        </div>
      )}
      {[...Array(pagination?.total)].map((e, i) => {
        ...
        if (i === pagination?.current) {
          return (
            <div className="pagination-item pagination-item--active" key={e}>
              {i}
            </div>
          );
        }
        return (
          <div onClick={() => setPage(i)} className="pagination-item" key={e}>
            {i}
          </div>
        );
      })}
